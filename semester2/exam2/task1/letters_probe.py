...
assert (alpha1.consonants == ['m','r','c','l','s','l'])  # consonants are in the original order
assert (alpha1.vowels == ['i','a','u','o','u','y'])  # but vowels are in the original order
...
assert (str(alpha3) == "First(consonants=['m', 'r', 'c', 'l', 's'], vowels=['i', 'a', 'u', 'o', 'u'])")
...
assert (str(beta1) == "First(consonants=['m', 'r', 'c', 'l'], vowels=['i', 'a', 'e'])") # use __class__, __bases__, __name__ attribute

# only Second's object can call encoder:
beta2 = beta1.encoder()  # so	instead	of 'miracle' it's now "rnwfhqj" by shift using
assert (str(beta2) == "First(consonants=['r', 'w', 'h', 'q', 'n', 'f', 'j'], vowels=[])") # encoding consonants then vowels
